...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self=[super initWithFrame:frame]) {
        ...
        [plusBtn setImage:[UIImage imageNamed:@"play"] forState:UIControlStateNormal];
        [plusBtn setImage:[UIImage imageNamed:@"pause"] forState:UIControlStateSelected];
        plusBtn.layer.cornerRadius = 25;
        plusBtn.layer.masksToBounds = YES;
        plusBtn.layer.borderColor = [UIColor whiteColor].CGColor;
...
//    self.plusBtn.centerX = [UIScreen mainScreen].bounds.size.width / 2;
//    //调整发布按钮的中线点Y值
//    self.plusBtn.centerY = self.height * 0.5 - 2*LBMagin ;
    CGFloat BtnX = [UIScreen mainScreen].bounds.size.width / 2 - 25;
    CGFloat BtnY = self.height * 0.5 - 2*LBMagin - 25;

//    self.plusBtn.size = CGSizeMake(50, 50);
    
    self.plusBtn.frame = CGRectMake(BtnX, BtnY, 50, 50);
